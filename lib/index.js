...
module.exports = function (cmd) {
    const {source, target, warn, processed, merge, file, configPath = '/', distPath = '/'} = cmd;
    ...
    Promise.resolve()
        ...
        .then((result)=>{
            ...
            console.log(`\nthe result file is ${fileName},and the path is ${cwd + realDistPath + `${fileName}.sql`}\n`);
            if (processed === 'true') {
                ...
                fs.writeFileSync(path.resolve(cwd, realDistPath + `${fileName}.sql`), dealFunc(result, externalFunc, warn));
                return;
            }
            fs.writeFileSync(path.resolve(cwd, realDistPath + `${fileName}.sql`), JSON.stringify(result));
        })
        .then(()=>{
