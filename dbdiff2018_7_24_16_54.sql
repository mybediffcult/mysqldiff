...
CREATE TABLE `effect_material` (
  ...
  `uid` int(11) unsigned NOT NULL COMMENT '创建者id',
  `uuid` varchar(128) DEFAULT NULL COMMENT '唯一id',
  `name` varchar(128) NOT NULL DEFAULT '' COMMENT '素材名称',
  `type` tinyint(4) NOT NULL DEFAULT '0' COMMENT '素材类型 0：静态图片 1：序列帧',
  `file_uri` varchar(256) NOT NULL DEFAULT '' COMMENT '素材文件地址',
  `icon_uri` varchar(256) NOT NULL DEFAULT '' COMMENT '素材缩略图文件地址',
  `sorting` text,
  `status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '状态 0：删除 1：未上线（剩余素材） 2：已发布素材',
  `sdk_version` int(11) NOT NULL COMMENT '最低兼容的 sdk 版本',
  `size` int(11) DEFAULT '0' COMMENT '素材文件大小',
...
ALERT TABLE `_app_effect_del` ADD INDEX `IDX_ANDROID_VERSION` USING BTREE (`android_version`),ADD INDEX `IDX_APP_CHANNEL` USING BTREE (`app_channel`),ADD INDEX `IDX_APP_CHANNEL_KEY` USING BTREE (`app_channel_key`),ADD INDEX `IDX_APP_EFFECT` USING BTREE (`app_id`,`app_channel`,`effect_order`),ADD INDEX `IDX_IOS_VERSION` USING BTREE (`ios_version`),DROP INDEX `IDX_APP_EFFECT2` ,DROP COLUMN `album_id`,DROP COLUMN `android_model`,DROP COLUMN `android_type`,DROP COLUMN `children`,DROP COLUMN `effect_desc_icon`,DROP COLUMN `effect_file_types`,DROP COLUMN `ios_model`,DROP COLUMN `ios_type`,DROP COLUMN `parent_id`,DROP COLUMN `related_words`,DROP COLUMN `sdk_version`,MODIFY `effect_type` tinyint(4) NOT NULL  COMMENT '特效类型';
ALERT TABLE `app_effect` MODIFY `children` text NULL  COMMENT '子集合的appeffectid',MODIFY `effect_description` varchar(128) NULL  COMMENT '特效提示文案',MODIFY `effect_file_types` varchar(256) NULL  COMMENT '特效种类，解析贴纸文件得到，逗号分隔',MODIFY `effect_icon` varchar(256) NULL  COMMENT '特效图标',MODIFY `ios_model` text NULL  COMMENT 'ios机型1',MODIFY `parent_id` bigint(11) NULL DEFAULT '0'  COMMENT '父级appeffectid',MODIFY `publish_time` datetime NULL  COMMENT '上线时间',MODIFY `related_words` text NULL  COMMENT 'appeffect的关联词';
...
ALERT TABLE `effect_material` ADD CONSTRAINT foo PRIMARY KEY (`id`);
